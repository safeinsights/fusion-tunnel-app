...
import { toWebRequest, sendWebResponse, MAX_BODY_BYTES, PayloadTooLargeError } from './adapter'
...
describe('toWebRequest', () => {
    it('maps method/url/headers (string, array, undefined) and buffers the body', async () => {
        ...
        expect(new URL(webReq.url).pathname).toBe('/v1/request')
    ...
    it('omits the body for GET requests', async () => {
        const req = fakeReq({ method: 'GET', url: '/health', headers: { host: 'localhost' } })
        const webReq = await toWebRequest(req)
    ...
    it('defaults the method to GET and the host to localhost', async () => {
        const req = fakeReq({ method: undefined as unknown as string, url: '/health', headers: {} })
        const webReq = await toWebRequest(req)
        expect(webReq.method).toBe('GET')
        expect(new URL(webReq.url).host).toBe('localhost')
    })
})
...
describe('toWebRequest body limits', () => {
    it('rejects a declared Content-Length over the limit without reading the body', async () => {
        const req = fakeReq({
            method: 'POST',
            url: '/v1/request',
            headers: { host: 'localhost', 'content-length': String(MAX_BODY_BYTES + 1) },
            body: 'a token amount, since the declared length is what gets rejected',
        })

        await expect(toWebRequest(req)).rejects.toThrow(`Request body exceeds the ${MAX_BODY_BYTES} byte limit`)
